...
if database:get('bot:forward:mute'..msg.chat_id_) then
  ...
  return 
	...
	if database:sismember(hash, result.sender_user_id_) then
         send(msg.chat_id_, msg.id_, 1, '_User_ *'..result.sender_user_id_..'* _is Betaeady moderator._', 1, 'md')
	else
	...
	if database:sismember(hash, result.sender_user_id_) then
         ...
         database:sadd(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '_User_ *'..result.sender_user_id_..'* _Added to Beta admins._', 1, 'md')
	end
    end
	...
	if result.id_ then
            ...
            texts = '<b>User </b><code>'..result.id_..'</code> <b>Added to Beta admins.!</b>'
            else 
            texts = '<code>User not found!</code>'
    end
	         send(msg.chat_id_, msg.id_, 1, texts, 1, 'html')
	...
	send(msg.chat_id_, msg.id_, 1, '_User_ *'..ap[2]..'* _Added to Beta admins._', 1, 'md')
    end
	-----------------------------------------------------------------------------------------------
	...
	if not database:sismember(hash, result.sender_user_id_) then
         ...
         database:srem(hash, result.sender_user_id_)
         send(msg.chat_id_, msg.id_, 1, '_User_ *'..result.sender_user_id_..'* _Removed from Beta Admins!._', 1, 'md')
	end
    end
	...
	if result.id_ then
         database:srem(hash, result.id_)
            texts = '<b>User </b><code>'..result.id_..'</code> <b>Removed from Beta Admins!</b>'
            else 
            texts = '<code>User not found!</code>'
    end
	         send(msg.chat_id_, msg.id_, 1, texts, 1, 'html')
	...
	local ap = {string.match(text, "^[#/!](remadmin) (%d+)$")} 	
         database:srem(hash, ap[2])
	send(msg.chat_id_, msg.id_, 1, '_User_ *'..ap[2]..'* Removed from Beta Admins!_', 1, 'md')
end
  ------------------ADD-REM-----------------------
if text:match("^[#!/]add$") and is_sudo(msg) then
	local hash = database:get('BGP:'..msg.chat_id_)
	if not hash then
	  database:set('BGP:'..msg.chat_id_,true)
	  send(msg.chat_id_, msg.id_, 1, 'Group has been added', 1, 'md')
	else
	  send(msg.chat_id_, msg.id_, 1, 'Group is already added', 1, 'md')
end
end

if text:match("^[#!/]rem$") and is_sudo(msg) then
  local hash = database:get('BGP:'..msg.chat_id_)
  if hash then
    database:del('BGP:'..msg.chat_id_,true)
    send(msg.chat_id_, msg.id_, 1, 'Group has been removed', 1, 'md')
  else
    send(msg.chat_id_, msg.id_, 1, 'Group has not added', 1, 'md')
end
end
	-----------------------------------------------------------------------------------------------
